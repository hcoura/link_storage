...
class LinkStorage:
    ...
    def new_link(self, url):
        try:
            self.cursor.execute("INSERT INTO %s(url) VALUES (%s)", (self.table, url))
            self.conn.commit()
    ...
    def set_crawled(self, url):
        try:
            self.cursor.execute("UPDATE %s SET crawled=TRUE  WHERE url=%s", (self.table, url))
            self.conn.commit()
        except Exception as e:
            self.conn.rollback()
    ...
    def set_tried(self, url_list):
        try:
            self.cursor.execute("UPDATE %s SET tries = tries + 1  WHERE url IN %s", (tuple(url_list),))
            self.conn.commit()
        except Exception as e:
            self.conn.rollback()
